...
### Added
- Ring-pipeline timing instrumentation to diagnose capture latency — each ring now logs `proxy_ms` (the HA `camera_proxy` round-trip), `capture_ms` (full capture step), and `pipeline_ms` (total pipeline wall time)
...
### Removed
- Lovelace custom card (`whorang-card.js`) — removed entirely due to HA ingress authentication issues
- `image_url` attribute from `sensor.doorbell_last_event` — was only needed for the card
...
### Fixed
- `whorang-card`: "WhoRang add-on not found" error on first load — the card now resolves the full Supervisor slug (e.g. `a48cb117_whorang`) via `hass.panels` before calling the Supervisor API, fixing discovery for custom-repo installs.
...
### Changed
- **Add-on slug renamed** from `doorbell_face_recognition` to `whorang` — the hostname becomes `a48cb117-whorang`. **Breaking change for existing installations:** uninstall the old add-on and reinstall. Data in `/share/doorbell` is preserved.
...
### Fixed
- **Person detail modal** — clicking a known person now works; added missing `GET /api/persons/{id}` endpoint that the modal was calling (previously only `GET /api/persons` existed, returning all persons)
...
### Changed
- **Removed model setting** — the llmvision integration already has the model configured per provider; no longer sending a `model` field in the service call, letting llmvision use its own default

### Removed
- `llmvision_model` setting from config, settings API, and Settings UI
...
### Fixed
- **llmvision 400 "caller did not ask for responses"** — `return_response` must be a query parameter (`?return_response=true`), not a JSON body field; moved it to the URL
...
### Fixed
- **README.md (Info tab)** — rewrote the addon info shown on the Info tab in HA; the previous content described the removed dlib face recognition system, incorrect sensors, wrong config options, and an outdated automation format
