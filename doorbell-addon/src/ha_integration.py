...
class HomeAssistantIntegration:
    ...
    async def register_entities(self):
        ...
        for entity in entities:
            try:
                ...
                logger.error("Failed to register entity", entity_id=entity["entity_id"], error=str(e))
    ...
    async def _register_entity(self, entity: Dict[str, Any]):
        ...
        await self.ha_api.update_sensor(entity["entity_id"], entity_data["state"], entity_data["attributes"])
    ...
    async def handle_face_detected(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            await self.ha_api.fire_event("doorbell_face_detected", {
            ...
            if event_data.get("primary_person_id"):
                await self.ha_api.fire_event("doorbell_known_person", {
                    "event_id": event_data.get("event_id"),
                ...
                await self.ha_api.fire_event("doorbell_unknown_person", {
                    "event_id": event_data.get("event_id"),
                    "faces_detected": event_data.get("faces_detected", 0),
                    "timestamp": event_data.get("timestamp")
                })
            
            logger.info("Face detection event processed", event_id=event_data.get("event_id"))
    ...
    async def register_device(self):
        ...
        device_info = {
            ...
            "configuration_url": f"http://localhost:8099"
        }
    ...
    async def create_automation_examples(self):
        ...
        for automation in automations:
            logger.info("Example automation available", 
