...
class DatabaseManager:
    ...
    def __init__(self):
        ...
        self.encryption_key = self._get_or_create_encryption_key() if settings.database_encryption else None
        self._init_database()
    ...
    def add_face_encoding(self, person_id: int, encoding: np.ndarray, confidence: float = 0.0) -> FaceEncoding:
    ...
    def get_face_encodings(self, person_id: Optional[int] = None) -> List[FaceEncoding]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if person_id:
                cursor = conn.execute(
                    "SELECT * FROM face_encodings WHERE person_id = ?",
                    (person_id,)
                )
            else:
    ...
    def add_doorbell_event(self, image_path: str, person_id: Optional[int] = None, 
                          confidence: Optional[float] = None) -> DoorbellEvent:
    ...
    def get_doorbell_events(self, limit: int = 100, offset: int = 0, 
                           person_id: Optional[int] = None) -> List[DoorbellEvent]:
